...
Charlotte.nhoods$npa <- as.factor(Charlotte.nhoods$npa)
...
ggplot() + 
  geom_sf(data=st_union(Charlotte.nhoods)) +
  geom_sf(data=Charlotte.Challenge) +
  geom_sf(data=med.buffer, color = "orange") +
  mapTheme()
...
reg.nhood <- lm(price ~ ., data = as.data.frame(Charlotte.training.nhood) %>% 
                                 dplyr::select(price, yearbuilt, heatedarea, 
                                               ...
                                               homo.Buffer, school.Buffer, med.Buffer, municipali, landusecod, shape_Area.x, npa))
...
Charlotte.Challenge <-
  Charlotte.Challenge %>%
  mutate(Price.Predict = predict(reg.nhood, Charlotte.Challenge)) %>%
  na.omit(Charlotte.test)

Charlotte.Challenge.Clean <-
  Charlotte.Challenge %>%
  dplyr::select(musaID, Price.Predict)
```
