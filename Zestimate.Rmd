...
We started the process by splitting the supplied data set into two groups: a larger one to supply information to our regression model, and smaller one to test whether that model is working. Then we worked through multiple iterations of the model using the interesting variables we'd gathered. 

Each regression gave us i
Regressionthe factors with their coeffecients collectively explain a dependent variable, in this case, the home price
These We started with collecting the essential parts of a hom


A couple rounds of iteration revealed 
